...
pres_2022_R1_bureaaux <- read_excel("./data-raw/2022/resultats-par-niveau-burvot-t1-france-entiere.xlsx", 
                                    guess_max = 70000) %>%
  mutate(Inscrits = as.numeric(Inscrits))

pres_22_R1_bureaux_cleaned <- lire(pres_2022_R1_bureaaux, 
                                    keep = c("Code du département", "Code de la commune", "Libellé de la commune", "Code du b.vote", 
                                             "Inscrits", "Abstentions", "Exprimés", "Blancs", "Nuls"), 
                                    col = seq(24, 101, 7), 
                                    gap = 2)


pres_2022_R1_bureaux_cleaned <- pres_22_R1_bureaux_cleaned %>% 
  ...
  select(CodeInsee, CodeDepartement, Commune = `Libellé de la commune`, `CodeBdeVote` = `Code du b.vote`, 
         Inscrits, Abstentions, Abstentions_ins, Votants, Votants_ins, 
...
readr::write_excel_csv(pres_2022_R1_bureaux_cleaned, path = "./data/P2022_Resultats_Bureaux_T1.csv")
rio::export(pres_2022_R1_bureaux_cleaned, file = "./data/P2022_Resultats_Bureaux_T1.xlsx")


head(pres_2022_R1_bureaux_cleaned)

pres_2022_R1_bureaux_cleaned %>% filter(Commybe=="38529") %>% View(.)
